...
\begin{figure}[H]
 \centering
    \input{figures/tikz_eisenhower_ideenspeicher}
...
in Ressourcen-\newline planung aufnehmen & \[\Bigg\{\begin{tabular}{cc} (\textit{K=tief,T=hoch}) & (\textit{K=mittel,T=hoch})  \\ (\textit{K=tief,T=sehr hoch}) & (\textit{K=mittel,T=sehr hoch}) \end{tabular}\Bigg\}\] & Der dritte Quadrant umfasst Ideen, die bezüglich der prognostizierten Kosten (Investitions- (\acrshort{capex} oder Betriebskosten (\acrshort{opex}) für die Realisierung als vertretbar eingestuft werden. Es wird jedoch erwartet, dass der Zeitaufwand für die Realisierung erheblich ist. Deshalb sollten Ideen dieses Quadranten in die Ressourcenplanung für die spätere Realisierung aufgenommen werden.\\
\hline
